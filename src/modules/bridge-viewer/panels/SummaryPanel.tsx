...
export function SummaryPanel({ data }: SummaryPanelProps) {
  ...
  const spanText = data.components.spans
    .map((span) => `${span.length} m`)
    .join(" + ");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="font-medium text-slate-100">
          {data.analysis.analysis_return_code}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span>
          {prettyNumber(
            data.params.total_length ??
              data.params.span_lengths.reduce((a, b) => a + b, 0),
          )}{" "}
          m
        </span>
      </div>
      ...
      <div className="grid gap-1 text-xs text-slate-300">
        <div>
          Min: [{data.global_bbox.min.map((v) => prettyNumber(v)).join(", ")}]
        </div>
        <div>
          Max: [{data.global_bbox.max.map((v) => prettyNumber(v)).join(", ")}]
        </div>
        <div>
          Size: [{data.global_bbox.size.map((v) => prettyNumber(v)).join(", ")}]
        </div>
